...
if (process.argv.length < 4) {
  console.error(
    `
Syntax: npx ts-node <className> <Key/Value pairs

  Example:
    npx ts-node po-codegen.ts "TestPageObject" "{ \"login\": \"input[name='login']\", \"password\": \"input[name='password']\", \"submitButton\": \"submit\" }"
`
  );
  process.exit(1);
}
